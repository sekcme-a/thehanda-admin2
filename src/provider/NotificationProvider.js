...
//   },
  const sendExpoSupabaseNotifications = async (
    ...
    options = {
      usePoint: true, onlyExpo: false, openDialog: true,
      customData: null //알림을 클릭했을 때 이동할 url 등 변경하려면
      //예) {url: `/(screen)/notification/${notificationId}`}
    }
  ) => {
    try{
      if(options.openDialog) setIsOpenDialog(true)

      if(teamId && options.usePoint) {
      ...
      for(const user of userList) {
        try{
          setProgress(`${user.displayName}(${user.realName})님에게 어플 알림을 전송합니다.`)
          ...
          await sendExpoNotification(pushToken, notificationId, notificationData, options?.customData)
    ...
    >
      <Dialog open={isOpenDialog} onClose={()=>setIsOpenDialog(false)}>
        ...
        >
          ...
          <p>{`성공: ${successes.length}, 실패: ${fails.length}, 대기: ${
            allCount-successes.length-fails.length}`}
          </p>
          <h4>{progress}</h4>
