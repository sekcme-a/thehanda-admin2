...
const Result = () => {
  ...
  const [isCustomNotificationDialogOpen, setIsCustomNotificationDialogOpen] =
    useState(false);
  ...
  const fetchData = async () => {
    try {
      ...
      const formData = postData.program_post_data.formData
        .map((item) => {
      ...
      const fetchedResult = await fetchResult(
        postId,
        postData.program_post_data.autoConfirm
      );
  ...
  const onConfirmClick = async () => {
    ...
    try {
      if (
        ...
        const isSendAlarm = confirm(
          "유저들에게 참여 확정 알림을 보내시겠습니까?\n취소 클릭 시, 알림 없이 참여 확정됩니다.\n(이미 참여 승인된 유저에게는 전송되지 않습니다.)"
        );

        //미승인 상태의 데이터만
        const sendList = result.filter(
          (item) => checkedList.includes(item.id) && item.confirmed === "미승인"
        );
...
해당 프로그램에 관심을 가져주셔서 감사합니다.`,
        ...
        for (let i = 0; i < confirmApplyIds.length; i++) {
          ...
          setProgress(
            `신청 승인 처리중입니다. (${i + 1}/${confirmApplyIds.length})`
          );
        }
...
신청 내용은 (마이페이지 > 신청 기록)에서 확인하실 수 있습니다.`,
      ...
      for (let i = 0; i < confirmApplyIds.length; i++) {
        ...
        setProgress(
          `신청 거절 처리중입니다. (${i + 1}/${confirmApplyIds.length})`
        );
      }
  ...
  const onParticipatedClick = async (isParticipate) => {
    const sendList = result.filter(
      (item) => checkedList.includes(item.id) && item.confirmed === "승인"
    );
    let nameList = "";
    sendList.map((item) => {
      nameList = `${nameList === "" ? "" : `${nameList}, `}${item.realName}(${
        item.displayName
      })`;
    });
    if (
      confirm(`총 ${sendList.length}명의 유저에 ${
        isParticipate ? "참여" : "불참"
      }처리 합니다.
...
${nameList}`)
      ...
      setIsFinished(false);
      setProgress(
        `${isParticipate ? "참여" : "불참"} 처리중입니다. (0/${
          confirmApplyIds.length
        })`
      );
      for (let i = 0; i < confirmApplyIds.length; i++) {
        await updateApplyParticipate(confirmApplyIds[i], isParticipate);
        setProgress(
          `${isParticipate ? "참여" : "불참"} 처리중입니다. (${i + 1}/${
            confirmApplyIds.length
          })`
        );
      }
  ...
  return (
    <>
      <Header title="신청 결과" />
      <div className="p-5">
        <h3 className="text-lg font-bold mb-1">폼 결과 확인</h3>
        <p className="text-sm font-bold mb-1">{`${post.title} 프로그램의 신청결과입니다.`}</p>
        {post.program_post_data.autoConfirm && (
          <p className="text-xs">*자동 승인이 적용된 프로그램입니다.</p>
        )}
        <Button
          variant="contained"
          size="small"
      ...
      <Dialog open={isConditionDialogOpen} onClose={onConditionDialogClose}>
        <p className=" bg-white pt-3 px-5">{progress}</p>
        <Button disabled={!isFinished} onClick={onConditionDialogClose}>
          확인
        </Button>
      </Dialog>
      <Dialog
