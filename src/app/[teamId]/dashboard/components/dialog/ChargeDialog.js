...
const ChargeDialog = () => {
  ...
  const [chargetPointAmountHelperText, setChargePointAmountHelperText] =
    useState(points.general);
  ...
  const onChargePointAmountChange = (e) => {
    ...
    if (e.target.value === "") setChargePointAmountHelperText(points.general);
      ...
      setChargePointAmountHelperText(
        parseInt(points.general) + parseInt(inputValue)
      );
  };
...
영업일 기준 1~3일 이내 소요됩니다.`
      ...
      setIsLoading(true);
      const { error } = await supabase.from("point_charge_apply").insert({
        team_id: teamId,
      ...
      const superAdminList = data.map((item) => ({ userId: item.user_id }));
      ...
      await sendExpoSupabaseNotifications(
        ...
        {
          title: "충전 신청",
          message: `새로운 충전 신청이 있습니다. 입금자 명: ${depositor}, 입금액: ${chargePointAmount}`,
        },
        null,
        {
  ...
  return (
    ...
    >
      ...
      />
      <p className="text-xs">
        입금 계좌: 기업 326-147145-04-011 주식회사 더한다
      </p>
